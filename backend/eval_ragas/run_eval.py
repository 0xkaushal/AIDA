...
Shares the same dataset as eval/ (backend/eval/dataset.py). Only the scoring
backend differs — RAGAS replaces the naive LLM-judge implementation.

Usage (from backend/):
    uv run python -m eval_ragas.run_eval --user_id <your_eval_user_id>
...
Options:
    ...
    --output            path to write JSON results (default: eval_ragas/results/run_<timestamp>.json)
    --skip-llm-metrics  skip RAGAS faithfulness + answer_relevancy (no extra LLM calls for scoring)
...
# ---------------------------------------------------------------------------
_HERE = Path(__file__).resolve().parent          # backend/eval_ragas/
...
from eval.dataset import DATASET, EvalCase  # noqa: E402  shared dataset
from eval_ragas.metrics import get_metrics  # noqa: E402
...
logger = logging.getLogger("aida.eval_ragas")
...
def main() -> int:
    ...
    # --- Step 4: summary ---
    def _avg(key: str):
        vals = [r[key] for r in all_results if r[key] is not None and str(r[key]) != "nan"]
